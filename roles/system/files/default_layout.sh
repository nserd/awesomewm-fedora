...
function two_monitors_layout_horizontal {
    /usr/bin/xrandr --listactivemonitors &> /dev/null
...
function two_monitors_layout_vertical {
    /usr/bin/xrandr --listactivemonitors &> /dev/null
    /usr/bin/xrandr --output eDP-1 --mode 1920x1080 --pos 0x1080 --rotate normal --output HDMI-1 --off --output HDMI-2 --primary --mode 1920x1080 --pos 0x0 --rotate normal
}

TRIGGER="$1"
HDMI_STATUS="$(cat /sys/class/drm/card1-HDMI-A-2/status)"

[ "$HDMI_STATUS" == "connected" ] && two_monitors_layout_vertical || one_monitor_layout
